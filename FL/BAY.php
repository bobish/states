...
try {
    ...
    {
	    $url = "http://records2.baycoclerk.com/Recording/Home/Index";
        $driver->get($url);
		...
		if(count($driver->findElement(WebDriverBy::id('resultsTable'))->findElement(WebDriverBy::tagName('tbody'))->findElement(WebDriverBy::tagName('tr'))->findElements(WebDriverBy::tagName('td'))) < 2)
		{
			...
			if(strpos($driver->getPageSource(), "Document Information") !== false)
			{
                ...
                sleep(1);
                while ($found == false) {	
                    // extract consideration amount each time for comparing0 
                    if(strpos($driver->getPageSource(), "Consideration1") !== false)
			        {
                        $consideration1 =trim(format_amt(space(explode('<td valign="top">',explode('</td>',explode('Consideration1</label>',$driver->getpageSource())[1])[1])[1])));
                    }else if(strpos($driver->getPageSource(), "Consideration</label>") !== false)
			        {
                    ...
                    if($consideration1 == $loan_amt)
                    {
						...
						// EXTRACTION
						if(strpos($original_instr, "-") !== false){
							$original_instr = str_replace("-", "", $original_instr);
						}
                        $docType =$docLegal =$grantor= $grantee=$book = $pg = $st = $recdt = $bktype = $bkpg = $insnum = $num = "";
					    $insnum =trim(space(str_replace("-", "", explode('<td valign="top">',explode('</td>',explode('Instrument #</label>',$driver->getpageSource())[1])[1])[1])));
                        ...
                        if(strpos($driver->getPageSource(), "Consideration1") !== false)
                        {
                            $consideration =trim(format_amt(space(explode('<td valign="top">',explode('</td>',explode('Consideration1</label>',$driver->getpageSource())[1])[1])[1])));
                        }else{
						...
						if(strpos($st,"Direct Name") !== false)
						{
                            $grantor  =trim(space(explode('<td valign="top">',explode('</td>',explode('Direct Name</label>',$driver->getpageSource())[1])[1])[1]));
						}	
						if(strpos($st,"Reverse Name") !== false)
						{
							$grantee =  trim(space(explode('<td valign="top">',explode('</td>',explode('Reverse Name</label>',$driver->getpageSource())[1])[1])[1]));
						}
						$avrData = array();
                        ...
                        // click next document
                        if(count($driver->findElements(WebDriverBy::id("directNavNext"))) > 0)
                        {
                            $pq++;
            ...
            if($found == true)
            {
                ...
                for($t=0; $t< $docCount; $t++)
                {
                    ...
                    sleep(3);
                    $driver->wait()->until(WebDriverExpectedCondition::visibilityOfElementLocated(WebDriverBy::id('imageLoadingImage')));
                    $driver->wait()->until(WebDriverExpectedCondition::invisibilityOfElementLocated(WebDriverBy::id('imageLoadingImage')));
                    // EXTRACTION 
                    $docType =$docLegal =$grantor= $grantee=$book = $pg = $st = $recdt = $bktype = $bkpg = $insnum = $num = "";
                    $insnum =trim(space(str_replace("-", "", explode('<td valign="top">',explode('</td>',explode('Instrument #</label>',$driver->getpageSource())[1])[1])[1])));
