...
try {
    ...
    {
	    $url = "http://landmark.clayclerk.com/LandmarkWeb/home/index";
        $driver->get($url);
        $driver->wait()->until(WebDriverExpectedCondition::visibilityOfElementLocated(WebDriverBy::xpath('//*[@id="bodySection"]/div/div/div[2]/div/div[1]/a/img')));
        $driver->findElement(WebDriverBy::xpath('//*[@id="topNavLinksSearch"]/a'))->click();
        if($first == true)
        {
		...
		sleep(2);
		$driver->findElement(WebDriverBy::xpath("//option[contains(text(), 'Show first 5000 records')]"))->click();
		sleep(1);
		...
		if(count($driver->findElement(WebDriverBy::id('resultsTable'))->findElement(WebDriverBy::tagName('tbody'))->findElement(WebDriverBy::tagName('tr'))->findElements(WebDriverBy::tagName('td'))) < 2)
		{
			...
			if(strpos($driver->getPageSource(), "Document Information") !== false)
			{
                ...
                while ($found == false) {	
                    ...
                    $consideration1 = format_amt($driver->findElement(WebDriverBy::xpath('//*[@id="documentInformationParent"]/table/tbody/tr[14]/td[2]'))->getText());
                    ...
                    if($consideration1 == $loan_amt)
                    {
                        ...
                        if(strpos($st ,"Book/Page")!== false)
                        {
                            $book = trim(str_replace(" o ","",str_replace(" O ","",space(explode("/",explode('">',explode("Book/Page</label>",$st)[1])[1])[0]))));
                            $pg = trim(space(explode("<",explode("/",explode('">',explode("Book/Page</label>",$st)[1])[1])[1])[0]));
                        }						
                        $recdt = trim(explode(" ",explode('">',explode("Record Date</label>",$st)[1])[1])[0]);
                        $bktype = trim(space(explode("<",explode('">',explode("Book Type</label>",$st)[1])[1])[0]));
                        $bkpg = trim(space(explode("<",explode('">',explode("Book/Page</label>",$st)[1])[1])[0]));
                        $insnum = trim($driver->findElement(WebDriverBy::xpath("//*[@id='documentInformationParent']/table/tbody/tr[1]/td[2]"))->getText());
                        $numpages = trim(space(explode("<",explode('">',explode("Number of Pages</label>",$st)[1])[1])[0]));
                        $docType = trim(space(explode("<",explode('">',explode("Doc Type</label>",$st)[1])[1])[0]));
                        ...
                        if(strpos($st,"Legal Description</label>") !== false)
                        {
                            $docLegal = trim(space(str_replace('</td>',"",str_replace('<div class="docNameSeperator"></div>',"",space(explode("</tr>",explode('top">',explode("Legal Description</label>",$st)[1])[1])[0])))));
                        }	
                        //echo $print = explode("</tr>",explode('top">',explode("Legal Description</label>",$st)[1])[1])[0];				
                        $grantor = trim(space(str_replace('</td>',"",str_replace('<div class="docNameSeperator"></div>',"",space(explode("</tr>",explode('top">',explode("Grantor</label>",$st)[1])[1])[0])))));
                        $grantee = trim(space(str_replace('</td>',"",str_replace('<div class="docNameSeperator"></div>',"",space(explode("</tr>",explode('top">',explode("Grantee</label>",$st)[1])[1])[0])))));
                    
                        $avrData = array();
                        $avrData["documentFileName"] = $insnum."_".$docType.".pdf";
            ...
            if($found == true)
            {
                ...
                for($t=0; $t< $docCount; $t++)
                {
                    ...
                    $driver->wait()->until(WebDriverExpectedCondition::invisibilityOfElementLocated(WebDriverBy::id('imageLoadingImage')));
                    // EXTRACTION 
                    $docLegal = $book = $pg = $st = $recdt = $bktype = $bkpg = $insnum3 = $num = "";
                    $st = $driver->getPageSource();
                    if(strpos($st ,"Book/Page")!== false)
                    {
                        $book = trim(str_replace(" o ","",str_replace(" O ","",space(explode("/",explode('">',explode("Book/Page</label>",$st)[1])[1])[0]))));
                    ...
                    $bktype = trim(space(explode("<",explode('">',explode("Book Type</label>",$st)[1])[1])[0]));
                    $bkpg = trim(space(explode("<",explode('">',explode("Book/Page</label>",$st)[1])[1])[0]));
                    $insnum3 = trim($driver->findElement(WebDriverBy::xpath("//*[@id='documentInformationParent']/table/tbody/tr[1]/td[2]"))->getText());
                    $numpages = trim(space(explode("<",explode('">',explode("Number of Pages</label>",$st)[1])[1])[0]));
                    $docType = trim(space(explode("<",explode('">',explode("Doc Type</label>",$st)[1])[1])[0]));
                    if(strpos($st,"Consideration1</label>") !== false)
                    {
                        $consideration = trim(space(format_amt(explode("</td>", explode('valign="top">', explode("Consideration1</label>", $st)[1])[1])[0])));
                    }else if(strpos($st,"Consideration</label>") !== false)
                    {
                        $consideration = trim(space(format_amt(explode("</td>", explode('valign="top">', explode("Consideration</label>", $st)[1])[1])[0])));
                    }else{
                        $consideration = 0;
                    }
                    if(strpos($st,"Legal Description</label>") !== false)
                    ...
                    $avrData = array();
                    $avrData["documentFileName"] = $insnum3."_".$docType.".pdf";
                    $avrData["documentType"] = $docType;
                    $avrData["book"] = $book;
                    $avrData["page"] = $pg;
                    $avrData["bookPage"] = $bkpg;
                    $avrData["bookType"] = $bktype;
                    $avrData["recordingDate"] = $recdt;
                    $avrData["instrumentNumber"] = $insnum3;
                    $avrData["numberOfPages"] = $numpages;
                    $avrData["considerationAmount"] = $consideration;
                    $avrData["legalDescription"] = $docLegal;
                    $avrData["grantor"] = $grantor;
