...
try {
    ...
    {
	    $url = "https://dory.escambiaclerk.com/LandmarkWeb1.4.6.134";
        $driver->get($url);
		...
		if(count($driver->findElement(WebDriverBy::id('resultsTable'))->findElement(WebDriverBy::tagName('tbody'))->findElement(WebDriverBy::tagName('tr'))->findElements(WebDriverBy::tagName('td'))) < 2)
		{
			...
			if(strpos($driver->getPageSource(), "Document Information") !== false)
			{
                ...
                while ($found == false) {	
                    ...
                    $consideration1 = format_amt($driver->findElement(WebDriverBy::xpath('//*[@id="documentInformationParent"]/table/tbody/tr[9]/td[2]'))->getText());
                    ...
                    if($consideration1 == $loan_amt)
                    {
                        ...
                        $grantor =trim(space(explode('<td valign="top">',explode('</td>',explode('Grantor</label>',$driver->getpageSource())[1])[1])[1])); 
                        $grantee =trim(space(explode('<td valign="top">',explode('</td>',explode('Grantee</label>',$driver->getpageSource())[1])[1])[1])); 
                        $avrData = array();
